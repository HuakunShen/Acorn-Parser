...
[Documentationon Notion](https://huakunshen.notion.site/Chrome-Extension-Template-with-TypeScript-Vue3-18c3c6c2c3a74b498afbcb3561777e59)
...
6. Use class-style component syntax: doesn’t matter
7. Use history mode for router: n (important)
8. CSS pre-processor: Sass/SCSS (with dart-sass)
9. Linkter / formatter config: ESLint + Prettier
   1. Link on save
10. Unit testing solution: Mocha + Chai
11. Config Files: In dedicated conig files
...
The `src` folder should be something like the following strucutre.
...
Since will have both a popup page and a options page, I replaced `App.vue` and `main.ts` with `popup.ts` + `Popup.vue` and `options.ts` + `Options.vue`.
...
Options points to `ui/options/index.html`.
...
## Interaction between popup and content.js

To have a better understanding, clone the code, try it, and read it.

This is how `content.ts` handles messages.

```jsx
chrome.runtime.onMessage.addListener((req, sender, sendResponse) => {
  switch (req) {
    case 'test':
      console.log('test from content.ts');
      sendResponse(true);
      break;
    default:
  }
...
The following code is added to a helper file.
...
The following code is a method in `Home.vue`. It will be triggered after clicking the button on the home page. The content script should receive the message and log a message in the browser console.
